...
from power  import gl as gl_ch, charging_deamon, charge
from power import get_bat
from power import chrg_isconnected as on_charge
...
from saving.py import save_sens_dock, save_sens_nodock
...
class Device:
    ...
    async def calibr(self):
        ...
        if touch[1]:
            ...
            sensor.sens = touch[0]
            save_sens_dock(touch[0])
        else:
            self._touch_conf_nodock = touch[0]
            save_sens_nodock(touch[0])
    ...
    @touch_conf_nodock.setter
    def set_nodock_sens(self, new_value):
        self._touch_conf_nodock = new_value
        
    @touch_conf_ondock.setter
    def set_ondock_sens(self, new_value):
        self._touch_conf_ondock = new_value        
...
with open('config.json', 'r') as jsonfile:
    ...
    if config_file["start_ftp"]:
        ...
        # Load touch sensetivy config 
        senscube.set_nodock_sens = config_file["nodock_sense"]
        senscube.set_ondock_sens = config_file["dock_sense"]
